...
document.addEventListener('DOMContentLoaded', () => {
    const animatedElements = document.querySelectorAll('.section-header, .feature-card, .about-content, .tech-content, .stats, .contact-content');
...
window.addEventListener('load', () => {
    ...
    if (heroDescription) {
        setTimeout(() => heroDescription.classList.add('fade-in-up'), 400);
    }
    if (heroButtons) {
        setTimeout(() => heroButtons.classList.add('fade-in-up'), 600);
    }
    if (heroImage) {
        setTimeout(() => heroImage.classList.add('fade-in-up'), 800);
    }
});
